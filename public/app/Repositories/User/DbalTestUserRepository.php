...
class DbalTestUserRepository implements UserRepository
{
    public function registerUserChoice(string $userName, int $quizId): int
    {
        ...
        return Database::connection()->lastInsertId('test_users');
    }
    ...
    public function getUserAnswers(int $userId, int $quizId): array
    {
        $stmt = Database::connection()
            ->prepare('SELECT * FROM `test_user_answers` WHERE `user_id` = ? AND `quiz_id` = ?');
